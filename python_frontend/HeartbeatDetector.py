...
from utilities import shrink_sparsities, shrink_per_row, shrink_per_col
...
class HeartbeatDetector():
    ...
    def get_graphs(
            self,
            nonparanormal: bool = False,
            teralasso: bool = False
        ) -> dict[str, np.ndarray]:
        """
        ...
        nonpara = None
        if nonparanormal:
            nonpara = nonparanormal_tensor_skeptic(self.heartbeat)
            if not teralasso:
                nonpara = {
                    "frames": nonpara[0],
                    "rows": nonpara[1],
        ...
        if not teralasso:
            self.Psis = GmGM(
                ...
                {'heartbeats': ('', 'frames', 'rows', 'columns')},
                gram_matrices=nonpara
            )
        else:
            if nonpara is None:
                self.Psis = TeraLasso(self.heartbeat, betas=[0., 0., 0.])
            else:
                self.Psis = TeraLasso_cov(nonpara, betas=[0., 0., 0.])
            self.Psis = {
                "frames": self.Psis[0],
                "rows": self.Psis[1],
                "columns": self.Psis[2]
